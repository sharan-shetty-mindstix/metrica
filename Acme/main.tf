...
resource "azurerm_storage_account" "storage" {
  name                = "st${substr(replace(lower(local.final_resource_name), "/[^a-z0-9]/", ""), 0, 22)}"
...
resource "azurerm_service_plan" "serverfarm" {
  name                = "plan-${local.final_resource_name}"
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = var.additional_tags
...
resource "azurerm_log_analytics_workspace" "workspace" {
  name                = "log-${local.final_resource_name}"
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = var.additional_tags
...
resource "azurerm_application_insights" "insights" {
  name                = "appi-${local.final_resource_name}"
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = var.additional_tags
...
resource "azurerm_windows_function_app" "function" {
  name                = "func-${local.final_resource_name}"
  resource_group_name = var.resource_group_name
  location            = var.location
  tags                = var.additional_tags
